...
class RideMyWayTestCase(unittest.TestCase):
    ...
    def test_user_can_create_ride_offer(self):
        res = self.client.post('app/v1/main/users/'+ self.user1.email+'/rides', data=json.dumps(self.rideOffer.__repr__()), headers = {'content-type': 'application/json'})
        self.assertEqual(res.status_code, 201)
        self.assertIn(res.data, self.user1.email)

    def test_user_can_view_ride_offers(self):
        res = self.client.get('app/v1/main/rides')
    ...
    def test_user_can_request_ride_offer(self):
        res = self.client.post('app/v1/main/ride_offers/ride_requests/', data=json.dumps(self.rideRequest.__repr__()), headers = {'content-type': 'application/json'})
        self.assertEqual(res.status_code, 201)
